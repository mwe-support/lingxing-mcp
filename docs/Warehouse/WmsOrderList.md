...
网页字段映射遵循实际导出语义：订单类型 `1/2/3` 分别为`单品单件/多品多件/单品多件`；收货地址优先使用 `consignee_full_address`；存在申报数据时，申报币种兼容商品明细的 `currency_code`；申报重量按网页格式输出为两位小数加 `g`。

当前 OpenAPI 返回不含网页端`库位`字段，该列保留为空并列入导出摘要的 `unavailable_columns`。
