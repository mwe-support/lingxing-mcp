...
官方单页上限为 1000。实测同一站点的 20 个启用美国店铺可正常返回，跨站点混合提交则可能返回空结果，而官方文档未说明该限制。MCP 因此先按 `marketplace_code` 在服务端分组，再对每组使用 `offset/length` 顺序翻页并合并结果。调用方仍只发起一次 MCP 请求，不需要按店铺、站点或页码批量调用工具。
